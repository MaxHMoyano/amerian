...
const StaffModal = ({ show, onClose, selected }) => {
  ...
  useEffect(() => {
    if (!_.isEmpty(selected)) {
      dispatch(
        staffActions.fetchStaffById(selected.hotel.id, selected.id)
      ).then((staff) => {
        Promise.all([
          dispatch(hotelActions.fetchHotel(staff.hotel)),
          dispatch(positionActions.fetchPosition(staff.position)),
        ]).then(([hotel, position]) => {
          formik.setValues({
          ...
          <Form.Row>
            ...
            <Col>
              <Form.Group>
                ...
                />
                {formik.errors.phone && formik.touched.phone && (
                  <span className='error_message'>{formik.errors.phone}</span>
                )}
              </Form.Group>
            </Col>
          </Form.Row>
