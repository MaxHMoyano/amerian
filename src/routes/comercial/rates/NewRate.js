...
const NewRate = () => {
  ...
  let corporationClients = useSelector(({ client }) =>
    client.results.filter((client) => client.type === 'COR')
  );
  let corpAgencyClient = useSelector(({ client }) =>
    client.results.filter((client) => client.type === 'COA')
  );
  let operatorClients = useSelector(({ client }) =>
    client.results.filter((client) => client.type === 'OPE')
  );
  let agencyClients = useSelector(({ client }) =>
    client.results.filter((client) => client.type === 'AGE')
  );
  ...
  let currentUserGroups = useSelector(({ user }) => user.current.rol);
  ...
  const roomTypeSchema = Yup.object().shape({
    ...
    currency: Yup.string().required(
      'El precio del tipo de habitacion es requerido'
    ),
  ...
  const rateSchema = Yup.object().shape({
    ...
    details: Yup.array().of(
      Yup.object().shape({
        name: Yup.string(),
        date_from: Yup.date()
          .required('La fecha desde principal es requerida')
          .nullable(),
        date_to: Yup.date()
          .required('La fecha hasta principal es requerida')
          .nullable(),
    ...
    alternative_currency: Yup.object()
      .shape({ label: Yup.string().required(), value: Yup.string().required() })
      .nullable(),
    exchange_rate: Yup.number().when(
  ...
  const formik = useFormik({
    ...
    onSubmit: (values, { setSubmitting, resetForm }) => {
      ...
      let rate = {
        ...
        alternative_currency: values.alternative_currency
          ? values.alternative_currency.value
          : null,
      ...
      dispatch(rateActions.createRate(values.hotel.value, rate)).then(
        (rate) => {
          // Creo los amounts por cada habitacion
          Promise.all(
            formik.values.roomTypes.map((e) =>
              dispatch(
                rateActions.createRateAmount(values.hotel.value, rate.id, {
                  roomCategory: e.id,
                  rate: rate.id,
                  amount: e.currency,
                })
            ...
            // Creo el detalle
            let details = values.details.map((e) =>
              dispatch(
                rateActions.createRateDetail(values.hotel.value, rate.id, {
                  date_from: formatISO(e.date_from).split('T')[0],
                  date_to: formatISO(e.date_to).split('T')[0],
                  alternative_date_from: e.alternative_date_from
                    ? formatISO(e.alternative_date_from).split('T')[0]
                    : null,
                  alternative_date_to: e.alternative_date_to
                    ? formatISO(e.alternative_date_to).split('T')[0]
                    : null,
                  rate: rate.id,
                  name: e.name,
                })
            ...
            Promise.all(details).then(() => {
              ...
              conditions = conditions.map((condition) =>
                dispatch(
                  rateActions.createRateCondition(
                    values.hotel.value,
                    rate.id,
                    condition
                  )
  ...
  const handleClientTypeChange = (e) => {
    ...
    let clientTypeIdx = activeExCopy.findIndex(
      (ex) => ex.name === e.target.id.split('-')[0]
    );
    ...
    activeExCopy[clientTypeIdx] = {
      ...
      useUsd:
        currency === 'USD'
          ? e.target.checked
          : activeExCopy[clientTypeIdx].useUsd,
      useArs:
        currency === 'ARS'
          ? e.target.checked
          : activeExCopy[clientTypeIdx].useArs,
    };
  ...
  const handleRoomTypeChange = (value, idx) => {
    ...
    if (formik.values.alternative_currency && formik.values.exchange_rate) {
      formik.setFieldValue(
        `roomTypes[${idx}].alternative_currency`,
        (value * formik.values.exchange_rate).toFixed(4)
      );
    }
  };
  ...
  const handleExchangeRateChange = (e) => {
    ...
    if (formik.values.alternative_currency) {
      formik.values.roomTypes.forEach((el, idx) => {
        formik.setFieldValue(
          `roomTypes[${idx}].alternative_currency`,
          (el.currency * e.target.value).toFixed(4)
        );
      });
    }
  };
  ...
  const hasErrors = (idx, key) => {
    if (formik.errors.details && formik.errors.details.length) {
      if (
        formik.errors.details[idx] &&
        formik.errors.details[idx][key] &&
        !_.isEmpty(formik.touched.details) &&
        formik.touched.details[idx] &&
        formik.touched.details[idx][key]
      ) {
        return true;
      }
    }
  ...
  const addNewDetail = (e) => {
    ...
    formik.setFieldValue('details', [newDetail, ...formik.values.details]);
  };
  ...
  const addNewException = (name) => {
    ...
    formik.setFieldValue(`${name}.exceptions`, [
    ...
    <Fragment>
      ...
      >
        <div className='d-flex align-items-center mb-5'>
          ...
          <h3 className='font-weight-bold text-primary mb-0 ml-2'>
            {getTitle()}
          </h3>
        </div>
        <Row>
          <Col md={6}>
            <Form.Group>
              ...
              <Select
                ...
                isDisabled={!hotels.results.length || currentHotel}
            ...
            <Row>
              <Col md={6}>
                <Form.Group>
                  ...
                  <Select
                    options={currencies.results.map((currency) => ({
                      label: currency.name,
                    ...
                    <div className='d-flex flex-column'>
                      <Datepicker
                        className={`form-control ${
                          hasErrors(0, 'date_from') ? 'is-invalid' : ''
                        }`}
                        placeholderText='Desde'
                        selected={formik.values.details[0].date_from}
                        onChange={(value) =>
                          formik.setFieldValue('details[0].date_from', value)
                        }
                        dateFormat={globalDateFormat}
                      />
                      {hasErrors(0, 'date_from') && (
                        <span className='error_message'>
                          {formik.errors.details[0].date_from}
                        </span>
                      )}
                    </div>
                    <div className='d-flex flex-column'>
                      <Datepicker
                        className={`form-control ${
                          hasErrors(0, 'date_to') ? 'is-invalid' : ''
                        }`}
                        selected={formik.values.details[0].date_to}
                        onChange={(value) =>
                          formik.setFieldValue('details[0].date_to', value)
                        }
                      ...
                      {hasErrors(0, 'date_to') && (
                        <span className='error_message'>
                          {formik.errors.details[0].date_to}
                        </span>
                      )}
                    </div>
                  </div>
              ...
              <Col md={6}>
                <Form.Group>
                  ...
                  <Select
                    ...
                    onChange={(value) =>
                      formik.setFieldValue('alternative_currency', value)
                    }
                    options={currencies.results.map((currency) => ({
                      label: currency.name,
                      value: currency.value,
                    ...
                    classNamePrefix='react_select'
                  />
                  {formik.errors.alternative_currency &&
                    formik.touched.alternative_currency && (
                      <span className='error_message'>
                        {formik.errors.alternative_currency}
                      </span>
                    )}
                ...
                <Form.Group>
                  ...
                  <div className='d-flex'>
                    <div className='d-flex flex-column'>
                      <Datepicker
                        className={`form-control ${
                          hasErrors(0, 'alternative_date_from')
                            ? 'is-invalid'
                            : ''
                        }`}
                        placeholderText='Desde'
                        selected={
                          formik.values.details[0].alternative_date_from
                        }
                        onChange={(value) =>
                          formik.setFieldValue(
                            'details[0].alternative_date_from',
                            value
                          )
                        }
                        dateFormat={globalDateFormat}
                      ...
                      {hasErrors(0, 'alternative_date_from') && (
                        <span className='error_message'>
                          {formik.errors.details[0].alternative_date_from}
                        </span>
                      )}
                    </div>
                    <div className='d-flex flex-column'>
                      <Datepicker
                        className={`form-control ${
                          hasErrors(0, 'alternative_date_to')
                            ? 'is-invalid'
                            : ''
                        }`}
                        placeholderText='Hasta'
                      ...
                      {hasErrors(0, 'alternative_date_to') && (
                        <span className='error_message'>
                          {formik.errors.details[0].alternative_date_to}
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <Form.Group>
                    ...
                    <div className='d-flex'>
                      <div className='d-flex flex-column'>
                        <Datepicker
                          className={`form-control ${
                            hasErrors(idx, 'date_from') ? 'is-invalid' : ''
                          }`}
                          placeholderText='Desde'
                          selected={formik.values.details[idx].date_from}
                          onChange={(value) =>
                            formik.setFieldValue(
                              `details[${idx}].date_from`,
                              value
                            )
                          }
                          dateFormat={globalDateFormat}
                        />
                        {hasErrors(idx, 'date_from') && (
                          <span className='error_message'>
                            {formik.errors.details[idx].date_from}
                          </span>
                        )}
                      </div>
                      <div className='d-flex flex-column'>
                        <Datepicker
                          className={`form-control ${
                            hasErrors(idx, 'date_to') ? 'is-invalid' : ''
                          }`}
                          placeholderText='Hasta'
                          ...
                          dateFormat={globalDateFormat}
                        />
                        {hasErrors(idx, 'date_to') && (
                          <span className='error_message'>
                            {formik.errors.details[idx].date_to}
                          </span>
                        )}
                      </div>
                    </div>
        ...
        <div>
          <RoomsModal
            hotel={selectedHotel}
            show={showRoomsModal}
            onClose={(e) => setShowRoomsModal(false)}
          />
          <h3 className='text-muted mt-5'>
            Categorias de habitaciones y Racks
          </h3>
          <Table hover bordered>
            <thead>
              <tr>
                ...
                {formik.values.alternative_currency && (
                  <th>
                    <span className='text-muted'>
                      {formik.values.alternative_currency.label}
                    </span>
                  </th>
                )}
              </tr>
                    ...
                    <td>
                      <div className='d-flex flex-column'>
                        <Form.Control
                          ...
                          onChange={(e) =>
                            handleRoomTypeChange(e.target.value, idx)
                          }
                        />
                  ...
                  {formik.values.alternative_currency && (
                    <td>
                      <Form.Control
                        ...
                        value={
                          formik.values.roomTypes[idx].alternative_currency
                        }
                      />
                    </td>
                  )}
        ...
        <p className='text-muted mb-0 mt-4'>
          Tipos de tarifas disponibles en el hotel:{' '}
        ...
        <div className='d-flex'>
          {clientTypes.map((clientType, idx) => (
            <div
              ...
              className={` ${
                idx === 0 ? 'pr-4' : 'px-4'
              } d-flex flex-column py-2 border-right`}
            >
        ...
        {isActive('COR') && (
          <div className='bg-gray p-4 mt-5'>
            ...
            <p className='text-muted'>
              <strong>Excepciones: </strong> Rango recomendado es entre el 17% y
              el 21%
            </p>
              ...
              <Row className='mb-2' key={idx}>
                <Col md={8}>
                  <Select
                    ...
                    onChange={(value) =>
                      formik.setFieldValue(
                        `COR.exceptions[${idx}].names`,
                        value
                      )
                    }
                    isMulti
                    options={corporationClients.map((client) => ({
                      label: client.name,
                      value: client.id,
                    }))}
                ...
                >
                  ...
                  <i
                    onClick={(e) => formik.values.COR.exceptions.splice(idx)}
                    className='fas fa-trash mx-3 icon-button'
                  ></i>
                  <Button
                    variant='outline-info mx-1'
                    onClick={(e) => addNewException('COR')}
                  >
                    <i className='fas fa-plus'></i>
                  </Button>
                </Col>
              ...
              <Row className='mb-2' key={idx}>
                <Col md={8}>
                  <Select
                    ...
                    onChange={(value) =>
                      formik.setFieldValue(
                        `OPE.exceptions[${idx}].names`,
                        value
                      )
                    }
                    isMulti
                    options={operatorClients.map((client) => ({
                      label: client.name,
                      value: client.id,
                    }))}
                    closeMenuOnSelect={false}
                ...
                >
                  ...
                  <span className='ml-1'>%</span>
                  <i
                    onClick={(e) => formik.values.OPE.exceptions.splice(idx)}
                    className='fas fa-trash mx-3 icon-button'
                  ></i>
                  <Button
                    variant='outline-info mx-1'
                    onClick={(e) => addNewException('OPE')}
                  >
                    <i className='fas fa-plus'></i>
                  </Button>
                </Col>
              ...
              <Row className='mb-2' key={idx}>
                <Col md={8}>
                  <Select
                    ...
                    onChange={(value) =>
                      formik.setFieldValue(
                        `AGE.exceptions[${idx}].names`,
                        value
                      )
                    }
                    isMulti
                    options={agencyClients.map((client) => ({
                      label: client.name,
                      value: client.id,
                    }))}
                    closeMenuOnSelect={false}
                ...
                >
                  ...
                  <span className='ml-1'>%</span>
                  <i
                    onClick={(e) => formik.values.AGE.exceptions.splice(idx)}
                    className='fas fa-trash mx-3 icon-button'
                  ></i>
                  <Button
                    variant='outline-info mx-1'
                    onClick={(e) => addNewException('AGE')}
                  >
                    <i className='fas fa-plus'></i>
                  </Button>
                </Col>
        ...
        {isActive('COA') && (
          <div className='bg-gray p-4 mt-5'>
            <Row>
              <Col md={6}>
                <span className='section_title'>
                  Descuentos Agencias Corporativas
                </span>
              </Col>
              <Col
                md={{ span: 2, offset: 4 }}
              ...
              <Row className='mb-2' key={idx}>
                <Col md={8}>
                  <Select
                    ...
                    onChange={(value) =>
                      formik.setFieldValue(
                        `COA.exceptions[${idx}].names`,
                        value
                      )
                    }
                    isMulti
                    options={corpAgencyClient.map((client) => ({
                      label: client.name,
                      value: client.id,
                    }))}
                    closeMenuOnSelect={false}
                ...
                >
                  ...
                  <span className='ml-1'>%</span>
                  <i
                    onClick={(e) => formik.values.COA.exceptions.splice(idx)}
                    className='fas fa-trash mx-3 icon-button'
                  ></i>
                  <Button
                    variant='outline-info mx-1'
                    onClick={(e) => addNewException('COA')}
                  >
                    <i className='fas fa-plus'></i>
                  </Button>
                </Col>
