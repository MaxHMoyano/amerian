...
const RateDetail = () => {
  ...
  const getRackPrice = (amount, condition) => {
    ...
    return (
      parseFloat((basePercentage / 100) * amount.amount) +
  ...
  const approveRate = () => {
    ...
    rateActions
      .partialUpdateRate(rate.hotel, rate.id, approvedRate)
      .then((update) => {});
  };
    ...
    <Fragment>
      ...
      <Row className='mb-4'>
        <Col className='d-flex align-items-center'>
          ...
          <h2 className='text-primary mb-0 ml-2'>
            {rate.type ? rate.type.name : ''}
          </h2>
          <Badge
            className='p-2 ml-4 text-white'
            variant={rate.status ? rate.status.code : 'dark'}
          >
            {rate.status && rate.status.name}
          </Badge>
        </Col>
      </Row>
      <Row>
          ...
          <Form.Group>
            ...
            <Select
              options={currencies.results
                .filter(
                  (e) =>
                    e.value === rate.currency ||
                    e.value === rate.alternative_currency
                )
                .map((e) => ({ label: e.name, value: e.value }))}
              value={{
                label: currencies.results.find((e) => e.value === rate.currency)
                  .name,
                value: currencies.results.find((e) => e.value === rate.currency)
                  .value,
              }}
            />
          </Form.Group>
                  ...
                  {rate.conditions.map(
                    (condition, idx) =>
                      condition.list.length > 0 && (
                        <th key={idx}>{getClientName(condition.name)}</th>
                      )
                  )}
                </tr>
              </thead>
              <tbody>
                <tr className='table-secondary'>
                  ...
                  {rate.conditions.map(
                    (condition, idx) =>
                      condition.list.length > 0 && (
                        <td key={idx}>{getPercentage(condition)} %</td>
                      )
                  )}
                </tr>
                {rate.amounts.map((amount, idx) => (
                  <tr key={idx}>
                    ...
                    {rate.conditions.map(
                      (condition, idx) =>
                        condition.list.length > 0 && (
                          <td key={idx}>$ {getRackPrice(amount, condition)}</td>
                        )
                    )}
                  </tr>
                ))}
          ...
          condition.list.length > 0 && (
            <Row key={idx} className='my-3'>
              <Col md={6} style={{ backgroundColor: '#fbfbfb' }}>
                <h4 className='section_title mt-4'>
                  {clientTypes.length
                    ? clientTypes.find((e) => e.value === condition.name).name
                    : ''}
                </h4>
                <div className='p-3' style={{ backgroundColor: '#D9EDF7' }}>
                  <p>
                    Descuento para {getClientName(condition.name)}{' '}
                    {getPercentage(condition)}% salvo las siguientes
                    excepciones:
                  </p>
                  {condition.list.map(
                    (el, idx) =>
                      el.clients.length > 0 && (
                        <Fragment key={idx}>
                          <Badge variant='dark' className='p-2 mx-2 text-white'>
                            {parseFloat(el.percentage).toFixed(1)} % :{' '}
                          </Badge>
                      ...
                      <tr>
                        ...
                        {condition.list.map((el, idx) => (
                          <th
                            ...
                            width='5%'
                            className={`${
                              !el.clients.length ? 'text-info' : ''
                            }`}
                          >
                            {' '}
                            {parseInt(el.percentage).toFixed(1)} %
                          </th>
                        ))}
                      </tr>
                    </thead>
