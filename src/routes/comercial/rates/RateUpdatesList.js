    ...
    <Fragment>
      ...
      {searchParams.status !== null && (
        <Alert
          className='my-3'
          variant='info'
          onClose={clearStateFromFilters}
          dismissible
        >
          <strong>Tarifas: </strong>
          <Badge variant='info' className='p-2 text-white'>
            {rates.count}
          </Badge>{' '}
          <span className='mr-4'>Resultados encontrados</span>
          <strong>Filtrando por: </strong>
          <Badge
            className='p-2 ml-3 text-white'
            variant={
              rateStates.find((e) => e.value === searchParams.status).code
            }
          >
